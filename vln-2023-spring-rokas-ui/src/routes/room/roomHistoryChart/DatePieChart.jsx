...
export default function DatePieChart({ history }) {
  const graphColors = [
    "#844389",
    "#CE5582",
    "#FD7E6D",
  ...
  const historyData = history.reduce((acc, story) => {
    const date = story.date.split("T")[0]; // Extract the date part
    if (acc[date]) {
      acc[date]++;
    } else {
      acc[date] = 1;
  ...
  return (
    <div className="room-date-chart-container">
      <Typography className="chart-title">Date</Typography>
